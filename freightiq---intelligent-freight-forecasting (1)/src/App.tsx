...
const MainAppContent: React.FC = () => {
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-[#071522] flex items-center justify-center text-white text-xs">
        <div className="animate-pulse flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-[#12A6A6] animate-ping" />
          <span>Loading PortIN Maritime Architecture...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      className={`flex h-screen font-sans antialiased overflow-hidden transition-colors duration-200 ${
        isLight ? 'bg-[#F6F8F5] text-[#1E293B]' : 'bg-[#071522] text-[#E8F0F5]'
      }`}
    >
      ...
      <div
        className={`flex-1 flex flex-col h-full overflow-hidden transition-colors duration-200 ${
          isLight ? 'bg-[#F6F8F5]' : 'bg-[#071522]'
        }`}
      >
        {/* Top Live Freight Ticker Bar */}
        <LiveTickerBar
          onSelectCorridor={c => {
            setSelectedCorridorForTrade(c);
        ...
        <main
          className={`flex-1 overflow-y-auto p-4 sm:p-6 lg:p-7 transition-colors duration-200 ${
            isLight ? 'bg-[#F6F8F5] text-[#1E293B]' : 'bg-[#071522] text-[#E8F0F5]'
          }`}
        >
